...
TEST_F(BlockFileCacheTest, file_cache_path_storage_parse) {
    ...
    }
    {
        ...
        ASSERT_TRUE(parse_conf_cache_paths(file_cache_path, cache_paths).ok());
        ASSERT_EQ(cache_paths.size(), 1);
        ASSERT_EQ(cache_paths[0].normal_percent, 40);
    ...
    }
    {
        ...
        std::string file_cache_path = "[{\"path\": \"memory\", \"total_size\":102400}]";
        std::vector<doris::CachePath> cache_paths;
        Status st = parse_conf_cache_paths(file_cache_path, cache_paths);
        config::enable_external_file_meta_disk_cache = old_enable_external_file_meta_disk_cache;
...
void fill_cache_blocks(BlockFileCache* cache, const std::string& key_material,
    ...
    for (int64_t offset = 0; offset < total_size; offset += 100000) {
        ...
        auto blocks = fromHolder(holder);
        ASSERT_EQ(blocks.size(), 1);
        assert_range(1, blocks[0], FileBlock::Range(offset, offset + 99999),
                     FileBlock::State::EMPTY);
        ASSERT_EQ(blocks[0]->get_or_set_downloader(), FileBlock::get_caller_id());
        download(blocks[0]);
        assert_range(2, blocks[0], FileBlock::Range(offset, offset + 99999),
                     FileBlock::State::DOWNLOADED);
    }
}

FileCacheSettings meta_eviction_test_settings() {
    FileCacheSettings settings;
    settings.ttl_queue_size = 3000000;
    settings.ttl_queue_elements = 30000;
    settings.query_queue_size = 3000000;
    settings.query_queue_elements = 30000;
    settings.index_queue_size = 1000000;
    settings.index_queue_elements = 10000;
    settings.disposable_queue_size = 1000000;
    settings.disposable_queue_elements = 10000;
    settings.meta_queue_size = 1000000;
    settings.meta_queue_elements = 10000;
    settings.capacity = 9000000;
    settings.max_file_block_size = 100000;
    settings.max_query_cache_size = 30;
    return settings;
}
...
TEST_F(BlockFileCacheTest, meta_queue_can_evict_data_cache_first) {
    if (fs::exists(cache_base_path)) {
        fs::remove_all(cache_base_path);
    }
    fs::create_directories(cache_base_path);
    ...
    ASSERT_EQ(cache.get_stats_unsafe()["disposable_queue_curr_size"], 1000000);
    ASSERT_EQ(cache.get_stats_unsafe()["index_queue_curr_size"], 1000000);
    ASSERT_EQ(cache.get_stats_unsafe()["ttl_queue_curr_size"], 3000000);
    ASSERT_EQ(cache.get_stats_unsafe()["meta_queue_curr_size"], 0);

    fill_cache_blocks(&cache, "file_meta", FileCacheType::META, 1000000);

    ASSERT_EQ(cache.get_stats_unsafe()["meta_queue_curr_size"], 1000000);
    ASSERT_EQ(cache.get_stats_unsafe()["normal_queue_curr_size"], 3000000);
    ASSERT_EQ(cache.get_stats_unsafe()["index_queue_curr_size"], 1000000);
    ASSERT_EQ(cache.get_stats_unsafe()["ttl_queue_curr_size"], 3000000);
    ASSERT_EQ(cache._evict_by_size_metrics_matrix[FileCacheType::NORMAL][FileCacheType::META]
                      ->get_value(),
              1000000);
...
TEST_F(BlockFileCacheTest, normal_queue_does_not_evict_meta_cache) {
    if (fs::exists(cache_base_path)) {
        fs::remove_all(cache_base_path);
    }
    fs::create_directories(cache_base_path);
    ...
    context.cache_type = FileCacheType::NORMAL;
    context.query_id = query_id;
    auto key = BlockFileCache::hash("normal_overflow");
    ...
    ASSERT_EQ(cache.get_stats_unsafe()["meta_queue_curr_size"], 1000000);
    ASSERT_EQ(cache.get_stats_unsafe()["normal_queue_curr_size"], 3000000);
    ASSERT_EQ(cache._evict_by_size_metrics_matrix[FileCacheType::META][FileCacheType::NORMAL]
                      ->get_value(),
              0);
...
TEST_F(BlockFileCacheTest, read_if_cached_returns_downloaded_meta_block) {
    if (fs::exists(cache_base_path)) {
        fs::remove_all(cache_base_path);
    }
    fs::create_directories(cache_base_path);
    ...
    ReadStatistics rstats;
    context.stats = &rstats;
    context.cache_type = FileCacheType::META;
    context.query_id = query_id;

    auto key = BlockFileCache::hash("cached_file_meta_payload");
    std::string payload = "serialized footer bytes";
    auto holder = cache.get_or_set(key, 0, payload.size(), context);
    auto blocks = fromHolder(holder);
    ASSERT_EQ(blocks.size(), 1);
    ASSERT_EQ(blocks[0]->get_or_set_downloader(), FileBlock::get_caller_id());
    ASSERT_TRUE(blocks[0]->append(Slice(payload.data(), payload.size())).ok());
    ASSERT_TRUE(blocks[0]->finalize().ok());

    std::string output(payload.size(), '\0');
    ASSERT_TRUE(cache.read_if_cached(key, 0, Slice(output.data(), output.size()), context).ok());
    ASSERT_EQ(output, payload);

    auto missing_key = BlockFileCache::hash("missing_file_meta_payload");
    Status missing_status =
            cache.read_if_cached(missing_key, 0, Slice(output.data(), output.size()), context);
    ASSERT_TRUE(missing_status.is<ErrorCode::NOT_FOUND>());
