...
/** Minimal GCS OAuth filesystem used by connector-owned file writes. */
class GcsFileSystem final : public FileSystem {
public:
    static Result<std::shared_ptr<GcsFileSystem>> create(
...
private:
    GcsFileSystem(std::map<std::string, std::string> properties, std::string id);
