...
- 已接通首个 copy-on-write `DELETE FROM table [WHERE predicate]` 切片：按同一 pinned snapshot 读取幸存行并原子替换 active files，`NULL` 谓词、无条件删空和 affected rows 已通过 path Delta 回归。当前仅支持整表范围，不支持 `PARTITION`、`USING`、CTE、`ORDER BY/LIMIT`；存在 row-filter/data-mask policy 时会明确拒绝。Unity external/catalog-managed 复用相同 overwrite 事务，但真实 Databricks DELETE E2E 仍待验证；完整 UPDATE、MERGE 尚未实现。
