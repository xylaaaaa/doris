...
- 已验证 external、catalog-managed append，以及 path/Unity managed create 的实验性样例；已接通 plugin connector `INSERT OVERWRITE`，Delta 通过同一事务的 remove+add 原子替换 active files，并在并发版本变化时 fail closed。这是后续 copy-on-write DELETE/UPDATE 的事务基础，尚不等于已经支持完整 UPDATE、DELETE、MERGE。
