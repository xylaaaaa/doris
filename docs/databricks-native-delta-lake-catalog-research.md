...
- 已接通 copy-on-write `UPDATE table SET ... [WHERE predicate]`：affected-row 预查询和重写都绑定同一 Delta version，支持别名、多列及表达式赋值、`IS NULL`、零匹配和无条件全表更新。当前不支持 `FROM`、CTE、子查询、非确定性谓词及 `ORDER BY/LIMIT`。DELETE/UPDATE 在存在 row-filter/data-mask policy 时会明确拒绝；Unity external/catalog-managed 复用相同 overwrite 事务，但真实 Databricks DML E2E 仍待验证，完整 MERGE 尚未实现。
